...
- 主导过千万级用户入口的大疆用户中心的重构以及性能优化工作，
- 有负责大疆售后 crm 维修管理系统的重构开发，
  - 带着团队将项目从第一个模块开始，逐步完善，为这个系统建立起了完善的前端工程体系，
  - 通过 eslint 以及低代码等手段提升了开发效率和代码质量
  - 抽离了大量通用组件，提升了系统一致性和可维护性
- 还独立设计交付过跨系统公告插件
- 参与过大疆前端 DevOps 建设
- 近期也主导公司的 报表 设计与开发
...
**第一段，背景和角色：**  
用户中心是大疆的 C 端登录系统，有着千万级的用户，日均 百万 登录请求。
所以当时重构最大的问题不是技术的选型，而是追求风险最低。
...
**第二段，我做了什么：**  
所以， 我没有推倒重来，没有使用公司已有的技术栈，也没有用当时比较火的 vue3，而是走增量演进：  
1）把 PC/移动两套代码通过响应式融合成一套，先把维护成本收敛；  
2）围绕性能链路做专项治理——包体积、关键资源优先级、主线程长任务、弱网兜底，逐项定位、逐项解决；  
3）项目打包上，做了拆包和缓存优化(Service Worker 强缓存)，三方脚本管控，JS 加载和执行顺序优化，图片升级 WebP 并做好兜底。

**第三段，结果：**  
多端统一后，维护成本下降；登录转化提升，首屏指标也有明显改善。并且在公司内部获得了不错的成绩。
...
**第一段背景：**  
RMS 是全球售后工单系统，日均处理工单 1W+，页面 100+，权限链路复杂。
我是前端负责人。痛点是：复杂度上来后，风格和质量不统一会让交付波动很大。
